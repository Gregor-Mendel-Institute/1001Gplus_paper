...
path.pannagram = '/Users/annaigolkina/Library/CloudStorage/OneDrive-Personal/iglab/tools/pannagram/inst/'
...
path.analysis = paste(path.base, '02_analysis_git/04_sv/', sep = '')
...
path.mafft = paste0(path.figures,'tmp_mafft/')

if (!dir.exists(path.mafft)) dir.create(path.mafft, recursive = TRUE)
...
i.cl = 31
message(i.cl)
sv.comp = names(sv.graphcomp$membership[sv.graphcomp$membership == i.cl])

seq.comp = sv.seqs[sv.comp]

for(i.seq in 2:length(seq.comp)){
  s1 = seq2nt(seq.comp[1])
  s2 = seq2nt(seq.comp[i.seq])
  dot.score = dotScore(s1, s2)
  if(dot.score[2] > dot.score[1]){
    seq.comp[i.seq] = nt2seq(revCompl(s2))
  }
}

tmp.fasta = paste(path.mafft, 'tmp.fasta', sep = '')
aln.fasta = paste(path.mafft, 'aln.fasta', sep = '')
writeFastaMy(seq.comp, tmp.fasta)

system(paste('mafft --op 5 --quiet --maxiterate 100 ', tmp.fasta, '>', aln.fasta,  sep = ' '))

alignment = readFastaMy(aln.fasta)

seqs.mx.cons = aln2mx(alignment)
p1 = msaplot(seqs.mx.cons)

p2 = msadiff(seqs.mx.cons, i.ref = 2)
