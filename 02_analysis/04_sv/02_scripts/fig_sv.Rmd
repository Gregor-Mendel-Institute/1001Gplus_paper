...
i.chr = 1
i.acc = 1
df = data.frame(pangenome = sv.all$beg[sv.all$chr == i.chr], 
                accession = sv.pos.beg[sv.all$chr == i.chr, i.acc])
df = df[df[,2] != 0,]
# plot(df[,1], df[,2])
...
p <- ggplot(df, aes(pangenome, accession)) + 
  ...
  geom_abline(intercept = res1$coefficients[1], slope = res1$coefficients[2], color = '#594A4E', alpha = 0.4, size = 2.5) +
  geom_abline(intercept = res2$coefficients[1], slope = res2$coefficients[2], color = '#594A4E', alpha = 0.4, size = 2.5) +
  ...
  annotate("text", x = min(df$pangenome), y = max(df$accession), 
             label = paste('Chromosome', i.chr), hjust = 0, vjust = 0.8) +
...
pdf(paste(path.figures, 'sv_coord_corresp_chr_',i.chr,'_acc_',colnames(sv.pos.beg)[i.acc],'.pdf', sep = ''), 
    width = 5, height = 3.75)
print(p)     # Plot 1 --> in the first page of PDF
dev.off()


saveRDS(p, paste(path.figures, 'sv_coord_corresp_chr_',i.chr,'_acc_',colnames(sv.pos.beg)[i.acc], '.rds', sep = ''), 
        compress = F)
...
pdf(paste(path.figures, 'sv_comb_pangen_coord_chr_',i.chr,'.pdf', sep = ''), width = 5.2, height = 6)
print(pp)
dev.off()
```
...
bin.width = seq(0, max(sv.all$end[sv.all$chr == i.chr]), 1000000)
...
for(i in 1:(length(len.cutoffs) - 1)){
  ...
  df.bins <- sv.all[sv.all$chr == i.chr,] %>%
    ...
    # Группируем по хромосомам, бинам и значениям single, затем считаем количество
...
g <- ggplot(df, aes(Var1, value)) + geom_line(size = 2) + theme_minimal() + 
  theme(legend.position='none',
        strip.text.y = element_text(angle = 0)) + 
...
pdf(paste(path.figures, 'sv_freq_hist.pdf', sep = ''), width = 3, height = 2)
print(g)     # Plot 1 --> in the first page of PDF
dev.off()
```
## Hist by len-groups
...
len.min = 15 
cols = c('#EDF1D6','#DDFFBB','#66bb6a', '#3e8c4c', '#cca64e', '#b07046', '#dd925f', '#a6719d', '#e198d4', 'pink')
...
p
# 
pdf(paste(path.figures, 'sv_freq_hist_length.pdf', sep = ''), width = 7, height = 4)
print(p)     # Plot 1 --> in the first page of PDF
dev.off()
...
pdf(paste(path.figures, 'sv_freq_length_composit.pdf', sep = ''), width = 5.2, height = 4)
print(p)     # Plot 1 --> in the first page of PDF
dev.off()
```
...
p <- ggplot() +
  ...
  theme_minimal() + scale_fill_manual(values=cols, name ='TE content') +
