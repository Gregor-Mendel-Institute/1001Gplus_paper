...
```{r}
id.chr4 = which(sv.all$chr == 4)
sv.pos.beg4 = sv.pos.beg[id.chr4,]
sv.pos.end4 = sv.pos.end[id.chr4,]
sv.all4 = sv.all[id.chr4,]


p1 = min(which(sv.pos.beg4[,1] > 17486218))
p2 = max(which(sv.pos.beg4[,1] < 17486482))

sv.all4[p1:p2,]

cbind(cbind(sv.pos.beg4[p1:p2,1:4], sv.pos.end4[p1:p2,1]), sv.all[id.chr4[p1:p2],])



s = seqinr::read.fasta('/Volumes/Samsung_T5/vienn/work_browser/pangenome.fasta')
...
p <- ggplot(df, aes(pangenome, accession)) + 
  ...
  annotate("text", x = min(df$pangenome), y = max(df$accession), 
             label = paste('Chromosome ', i.chr, '; Slope ', round(df[nrow(df),2]/df[nrow(df),1],2), sep = ''), 
           hjust = 0, vjust = 0.8) +
