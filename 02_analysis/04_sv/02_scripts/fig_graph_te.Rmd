...
x = read.table('/Volumes/Samsung_T5/vienn/00_paper/01_data/06_methylation/02_on_sv_v03/CG.methylation_level.12accessions.svs_v03.txt', header = 1, row.names = 1, check.names = F)
...
sv.se.meth.max = rowMeans(x)
sv.se.meth.max = rowMax(x)


plot(rowMeans(x), rowMax(x))
...
sv.se$meth = sv.se.meth.max[sv.se$gr]

sv.se$te = factor(sv.se$te, levels = names(te.cols))

g = ggplot(sv.se[(sv.se$len > 100) & !(is.na(sv.se$meth)),], aes(x = te, y = meth, fill = te)) +
  geom_violin() +
  # ylim(0, 25000) +
