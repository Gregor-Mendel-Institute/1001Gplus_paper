...
# for(i.target in comp.target[1:20]){
  ...
  set.seed(20)
  p.te <- ggnet2(g.sub, 
                 label = round((sv.graph$nodes.traits[g.sub.names,]$len / 1000),1), 
                 label.size = 2,
                 label.color = 'white',
                 edge.color = "black", 
...
p <- ggnet2(g.part, label = F, edge.color = "black", 
            # node.size = g.nodes.cnt[b.graph.names], 
            node.size = 1,
            color = sv.memb[g.part.names,]$prot,
            palette = g.cols,
            # mode = "kamadakawai"
...
cl.np.prot = as.numeric(names(cl.np.prot)[cl.np.prot > 5])
...
for(i.com in cl.np.prot){
  names = names(sv.graphcomp$membership[sv.graphcomp$membership == i.com])
  names.len = as.numeric(sapply(names, function(s) tail(strsplit(s, '\\|')[[1]], 1)))
  print(names.len)
  
  s = sv.seqs[[names[1]]]
  p = dotplot(s, s, 15, 9)
  p.list[[length(p.list) + 1]] = p
  seq.list[[length(seq.list) + 1]] = s
}
...
# for(i.target in comp.target[1]){
  ...
  p.freq <- ggnet2(g.sub, 
              ...
              # node.size = sv.graph$nodes.traits[g.sub.names,]$cnt, 
              node.size = 7,
              color = sv.memb[g.sub.names,]$indel,
              mode = 'kamadakawai',
